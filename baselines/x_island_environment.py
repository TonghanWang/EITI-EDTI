...
class x_Island:
	...
	def initialization(self, args):
		...
		self.n_dim = 2
		self.n_wolf = 1
		self.n_landmark = self.args.x_num_landmark
		...
		elif self.args.x_num_landmark == 16:
			self.landmarks = []
			location_list = [int(self.size * 0.2), int(self.size * 0.4), int(self.size * 0.6), int(self.size * 0.8)]
			for x in location_list:
				for y in location_list:
					self.landmarks.append([x, y])
			self.landmarks = np.array(self.landmarks)
		elif self.args.x_num_landmark == 25:
			self.landmarks = []
			location_list = [int(self.size * 0.1), int(self.size * 0.3), int(self.size * 0.5),
			                 int(self.size * 0.7), int(self.size * 0.9)]
			for x in location_list:
				for y in location_list:
					self.landmarks.append([x, y])
			self.landmarks = np.array(self.landmarks)
		else:
			self.landmarks = np.zeros((self.size, self.size))
	...
	def obs(self):
		...
		ans = [a_state, wolf]
		if self.is_view_landmark:
			if self.args.x_num_landmark <= 25:
				ans.append(self.landmark_visited)
			else:
				ans.append(self.landmarks.reshape(-1))
		if self.is_full_obs:
				...
				if self.agent_alive_n[agent_index]:
					for i, landmark in enumerate(self.landmarks):
						if not self.landmark_visited[i] and (landmark == self.state_n[agent_index]).all():
							landmark_num += 1
							rew += self.args.x_landmark_rew
							self.landmark_visited[i] = True
		else:
			for agent_index in range(self.n_agent):
				if self.agent_alive_n[agent_index]:
					x, y = self.state_n[agent_index]
					if self.landmarks[x][y] == 0:
						landmark_num += 1
						rew += self.args.x_landmark_rew
						self.landmarks[x][y] = 1
